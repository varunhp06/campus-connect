...
import { CreateFoundItemData, FoundItem } from '@/components/data/lostAndFound';
import {
  claimFoundItem,
  createFoundItem,
  removeFoundItem,
...
export default function FoundItemsScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showToast('Found item posted successfully', 'success');
      resetForm();
      setModalVisible(false);
    } catch (error) {
      showToast('Failed to post found item', 'error');
      console.error(error);
    } finally {
      setSubmitting(false);
    }
  ...
  const handleClaimItem = async (item: FoundItem) => {
    ...
    if (item.userId === currentUser.uid) {
      showToast('You cannot claim your own found item', 'warning');
      return;
    }

    showDialog({
      title: 'Claim Item',
      message: `Claim that ${item.itemName} is yours?`,
      buttons: [
  ...
  const handleRemove = async (item: FoundItem) => {
    showDialog({
      title: 'Remove Item',
      message: 'Have you found the original owner yourself?',
      buttons: [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Remove',
