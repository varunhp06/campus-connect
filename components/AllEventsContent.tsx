...
import { ActivityItem, fetchActivities } from './data/activities';
...
export const AllEventsContent: React.FC = () => {
  ...
  const generatePDF = async () => {
    if (activities.length === 0) {
      showToast('There are no events to export', 'info');
      return;
    }
      ...
      if (Platform.OS === 'android') {
        ...
        if (permissions.granted) {
          ...
          await FileSystem.StorageAccessFramework.createFileAsync(
            ...
            showToast('PDF saved to Downloads folder!', 'success');
          });
        } else {
          showToast('Cannot save file without permission', 'error');
        }
      } else {
